...
const InputDates = ({ methods }) => {
  const { control, setError, clearErrors, getValues, formState: { errors }, handleSubmit } = methods;
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="flex lg:flex-row flex-col lg:space-x-5 space-x-0">
        <div className="mt-3 flex flex-col relative h-11 w-full min-w-[200px]">
          ...
          {errors.dateOfBirth && (
            <span className="error-message mt-2 text-red-500">{errors.dateOfBirth.message}</span>
          )}
        </div>
        ...
        <div className="mt-3 flex flex-col relative h-11 w-full min-w-[200px]">
          ...
          {errors.startDate && (
            <span className="error-message mt-2 text-red-500">{errors.startDate.message}</span>
          )}
        </div>
      </div>
